...
void display_cylinderbar_wh_clip_rgb(KOORD_VAL xp, KOORD_VAL yp, KOORD_VAL w, KOORD_VAL h, PIXVAL color, bool dirty, bool vertical  CLIP_NUM_DEF)
{
	display_fb_internal(xp, yp, w, h, color, dirty, CR.clip_rect.x, CR.clip_rect.xx, CR.clip_rect.y, CR.clip_rect.yy);
	if (vertical) {
		display_blend_wh(xp, yp, min(3, w / 2), h, COL_WHITE, 15);
		display_blend_wh(xp + 1, yp, 1, h, COL_WHITE, 15);
		uint8 start = w * 2 / 3;
		for (uint8 i = start; i < w; i++) {
			display_blend_wh(xp + i, yp, 1, h, COL_BLACK, i * 25 / h);
		}
	}
	else {
