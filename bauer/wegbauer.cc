...
void way_builder_t::build_river()
{
	...
	if(  env_t::river_types>1  &&  last_water_tile!=start_n) {
		...
		if (to_the_sea.calc_route(welt, welt->lookup_kartenboden(route[last_water_tile].get_2d())->get_pos(), welt->lookup_kartenboden(route[0].get_2d())->get_pos(), &river_tester, 0, 1, 0, 0x7FFFFFFF, false)) {
