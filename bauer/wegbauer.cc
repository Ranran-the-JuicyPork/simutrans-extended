...
class fluss_test_driver_t : public test_driver_t
{
	...
	bool is_target(const grund_t *cur,const grund_t *) OVERRIDE { return cur->is_water()  &&  cur->get_hoehe()<=world()->get_groundwater(); }
};
...
void way_builder_t::build_river()
{
	...
	if(  env_t::river_types>1  ) {
		...
		if (to_the_sea.calc_route(welt, welt->lookup_kartenboden(route[last_common_water_tile].get_2d())->get_pos(), welt->lookup_kartenboden(route[0].get_2d())->get_pos(), &river_tester, 0, 1, 0, 0x7FFFFFFF, false)) {
